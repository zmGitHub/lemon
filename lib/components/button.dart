...
import 'package:lemon/common/lcontrol.dart';
import 'package:lemon/common/ldefine.dart';

/// [LButton] 图片相对与文字的位置
///
/// [LButton] The position of the picture relative to the text
enum ImageAlignment {
  /// 文字左边
  ///
  /// Left of text
...
/// LButton 提供了一个常用按钮效果的实现集合。支持配置圆角、各种特效、边角、边框以及 Loading等。
/// LButton 让开发者可以只熟悉一个按钮组件，就能实现常见的按钮。而无需掌握多种不同类型的按钮组件。
///
/// LButton provides a collection of commonly used button effects.
/// Support for configuring rounded corners, various special effects, corners, borders, and loading.
/// LButton allows developers to familiarize themselves with just one button component and implement common buttons.
...
class LButton extends StatefulWidget {
  ...
  LButton({
    ...
    this.height,
    this.corner,
    this.cornerStyle = LCornerStyle.round,
    this.strokeColor,
    this.strokeWidth,
    this.shadowColor,
    this.shadowOffset,
    ...
    this.surfaceStyle = LSurface.Flat,
    this.onHover,
    this.onPressedDown,
    this.onPressedUp,
...
class _LButton extends State<LButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    widget.controller?.addListener(_onParamsChange);
  }

  @override
  void didUpdateWidget(LButton oldWidget) {
