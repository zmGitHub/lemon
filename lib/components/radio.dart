...
/// [LRadio] 灵活精美的单选组件，使用 [LRadio] 可以快速构建出丰富而精彩的单选视图。
/// [LRadio] 允许开发者为整个单选组件的交互周期状态进行精准的配置，这对于现代应用程序是十分重要的。
/// 现在，我们用户总是期待自己的每一次交互都能获得反馈。而 [LRadio] 使得这种复杂的状态变化带来的视图变化的构建，变得异常简单。
///
/// [LRadio] Flexible and beautiful radio components, using [LRadio] can quickly build a rich and wonderful radio view.
/// [LRadio] allows developers to accurately configure the interaction cycle state of the entire radio component, which is very important for modern applications.
...
class LRadio<T> extends StatefulWidget {
  ...
  /// 如果在应用中有独特的设计，可以使用 [LRadio.custom] 来完成特殊样式的构建。[LRadio] 将帮助开发者完成状态管理的构建。
  ///
  /// By default, [LRadio] has a very flexible style.
  /// Developers do not need to configure [normal], [selected], [disableNormal], [disableSelected], and [hover] themselves.
  /// [LRadio] provides a wealth of configuration options to help developers quickly complete the construction of the view.
  /// If you have a unique design in your application, you can use [LRadio.custom] to complete the construction of special styles.
  /// [LRadio] will help developers complete the construction of state management.
  ///
...
class _Radio<T> extends State<LRadio<T>> {
...
/// 为 [LRadio] 设置圆角。详见 [LRadio] 默认构造函数中的 [corner] 参数
///
...
class LRadioCorner {
  ...
  LRadioCorner.all(double radius)
